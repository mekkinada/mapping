...
import Loader from "../doc-loader/Loader";
...
const useCreatPoint = () => {
  ...
  return useMutation(createPoint, {
    ...
    onError: (error, point, context) =>
      queryClient.setQueryData("points", context.snapshot),
...
function Map() {
  ...
  return (
    ...
    >
      ...
      <div ref={geocoderRef} className={styles.searchbox} />
      <Geocoder
        mapRef={mapRef}
        containerRef={geocoderRef}
        onViewportChange={handleViewportChange}
        mapboxApiAccessToken={process.env.NEXT_PUBLIC_MAPBOX_ACCESS_TOKEN}
        placeholder="Recherchez votre région"
      />
      ...
      {points.length === 0 ? (
        ...
        points.map((point, i) => (
          <div key={i}>
            ...
            {point.id === locationId && showPopup && (
              <PopupInfo point={point} handelClose={handelClosePopup} />
            )}
          </div>
        ))
      )}
